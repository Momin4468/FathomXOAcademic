...
## CLIENT. Client portal — a third scoped identity plane (BUILT · migration 0033, module 18)

- **client_account** — the client's own login (own credentials), 1:1 to a client party: `org_id`, `party_id` (unique), `login_id` citext unique (email or a client/student id), `password_hash`, `twofa_secret` (sealed), `status` (`CLIENT_ACCOUNT_STATUSES = invited|active|lead|deactivated`), `expires_at` (leads), provenance. Tenant-RLS. **client_refresh_token** (rotation/reuse-detection; no org column → no RLS, like `pf_refresh_token`). **client_message** (`party_id` thread, `sender` client|admin; tenant-RLS). `work_item += client_account_id` (client-submitted-draft provenance + the lead-promotion trigger).
- **Identity plane:** distinct token `typ` client_access/client_refresh (carries orgId+partyId), `ClientAuthGuard` + `@ClientRoute()` (the global business guard yields; the three planes' tokens never cross-authenticate). DB work runs under the BUSINESS RLS context scoped to the client party — **no new GUC** (the data is business data).
- **Redacted reads (caller-guarded definers `app_is_superadmin() OR app_current_party()=p_client`):** `client_works` (own jobs + billed/paid/due — never writer/margin/chain), `client_outstanding` (AR), `client_messages` (own thread), `client_auth_lookup` (pre-login). `leg_visibility` RLS is the backstop.
- **Inbound draft (propose→confirm):** `POST /client/requests` forces `source_party_id` from the token, work_state='draft', zero legs (never auto-priced); the admin confirms (work:approve) + prices. Optional brief via the file pipeline (kind `brief` → `work_item.brief_file_id`).
- **Lead lifecycle (public-funnel seam):** the `client_promote_lead_on_confirm` trigger promotes a `lead`→`active` on job confirm; `client_purge_expired_leads()` (daily @Cron + manual `client_portal:approve` endpoint) deletes unconverted expired leads.
- **Admin surface:** `/client-portal/*` on the business plane, gated by the new **`client_portal`** permission (a1/a3 all; a2 view). WhatsApp handoff = a `wa.me` link from `GET /client/config` (env `CLIENT_PORTAL_WHATSAPP`). `MODULES += client_portal` (18). Needs `FEATURE_CLIENT_PORTAL`.

## RESET. Self-service password reset — all three auth planes (BUILT · migration 0034)

- **password_reset_token** — ONE shared, **definer-only** table (RLS forced ON, **no policy + no `app_user` grant** → touched only by the SECURITY DEFINER fns below; modelled on `pf_link_token`): `id`, `plane` (`business|pf|client` check), `account_id` (the user_account/pf_account/client_account id; no FK — spans three tables), `token_hash` (sha256 of the raw token, unique; raw lives ONLY in the email link), `expires_at`, `used_at` (single-use; also set when superseded), `created_at`. The flow is **pre-auth** (no session/RLS context at request or consume), hence definer-only.
- **Definers:** `pwreset_request(plane, account_id, token_hash, expires_at)` invalidates any prior live token for the account then inserts the new one. `pwreset_consume_business/_pf/_client(token_hash, new_hash)` — atomic, owner-rights: find a live token (`used_at is null and expires_at > now()`) `for update`, mark it used, set the plane's account `password_hash = new_hash`, **revoke ALL the account's live refresh tokens** (a reset ends every session), write a completion audit row (`audit_log` for business/client by org_id; `pf_audit_log` for pf), return the account id (NULL → caller maps to a generic 400). bcrypt can't run in SQL → the app passes the hash. The plane discriminator (`where plane='…'`) prevents cross-plane consumption. `client_reset_lookup(login)` → `(id, status, expires_at, email=coalesce(party.contact_json->>'email', login_id))`.
- **API/web:** `PasswordResetService` (in `AuthModule`) + `@Public POST {/auth,/pf/auth,/client/auth}/request-reset` + `/reset`. `request` ALWAYS returns generic `{ok}` and runs lookup→mint→email OFF the response path (no timing oracle); `reset` failures collapse to one generic 400 (no enumeration). Rate-limited (best-effort, in-process): per-IP + per-identifier on request, per-IP on reset. Web: `forgot-password` + `reset-password` page per plane + six BFF routes; `middleware.ts` allowlists them. Needs env `WEB_BASE_URL`, `PASSWORD_RESET_TTL_MIN` (default 60); `EMAIL_ADAPTER=dev` (+ `EMAIL_OUTBOX_DIR`) to deliver.

## PF-PLANNER. Personal Finance planner tables (BUILT · migration 0035, module 14)

- **pf_preferences** — one row/account (unique `pf_account_id`), the PF settings ("sensible defaults, few visible settings"): `rollup_period` (week|month|custom) + `rollup_custom_days`, `subscription_lead_days` (default 3), `reminder_subscriptions`/`reminder_notes`/`anomaly_enabled` bools, `anomaly_threshold_pct` (sensitivity, default 150), `active_currencies text[]`, `default_budget_period`, `ai_quickadd_enabled`, `prefs_json`, `updated_at`. Lazily created with defaults on first read; base/default currency stays on `pf_account.base_currency` (not duplicated). Config, not a balance.
- **pf_anomaly_notice** — dedup + dismissible in-app alert log: `kind` (period_total|category), `period_key`, `category_id` (null for period_total), `observed`, `baseline`, `currency`, `created_at`, `dismissed_at`. Unique `(pf_account_id, kind, period_key, coalesce(category_id, nil-uuid))` = one notice per scope/period. A notification SNAPSHOT, NOT a stored money balance (all figures stay derived).
- **pf_ai_usage** — PF-scoped AI quick-add daily-cap counter: `(pf_account_id, day)` unique, `count`. Kept in the PF plane so a PF AI action never touches the business `ai_usage`/`ai_capture` tables.
- All three: account-scoped RLS (`enable`+`force`, `pf_account_isolation using (pf_account_id = app_current_pf_account())`, NO superadmin clause), grants select/insert/update to app_user. Same structural privacy as every pf_* table (business/SuperAdmin read zero rows).
- **Derived reads (no new stored money):** `GET /pf/insights?period=` (period-aware totals/category-breakdown/trend series, one shared resolver `pf-period.ts`), `GET /pf/categories/frequent`, the anomaly heuristic (current period vs avg of prior K periods), all computed at read. AI quick-add returns a DRAFT only (confirmed via `POST /pf/expense`). Anomaly reminder = a @Cron (09:45) mirroring the subscription/note reminders; `POST /pf/anomaly-notices/run` is the same-account manual trigger.
