...
/** Multipart import-preview seam — forwards the CSV/Excel + entity to the API. */
export async function POST(req: NextRequest) {
  ...
  const send = (token: string | undefined) =>
    fetch(`${API_URL}/import/preview`, { method: "POST", headers: token ? { authorization: `Bearer ${token}` } : {}, body: form });
