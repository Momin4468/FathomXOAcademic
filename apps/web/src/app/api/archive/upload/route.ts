...
/** Multipart archive-create seam — forwards the file + metadata to POST /archive. */
export async function POST(req: NextRequest) {
  ...
  const send = (token: string | undefined) =>
    fetch(`${API_URL}/archive`, { method: "POST", headers: token ? { authorization: `Bearer ${token}` } : {}, body: form });
