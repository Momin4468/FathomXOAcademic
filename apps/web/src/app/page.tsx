    ...
    <AppShell>
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">What&rsquo;s open and whose move is it?</p>
        </div>
      ...
      {owner && (
        <div className="mb-6 space-y-4">
          <Card>
            <h2 className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-500">Outstanding client dues</h2>
          ...
          <Card>
            <h2 className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Business margin (derived)</h2>
            <div className="grid grid-cols-3 gap-3 text-sm">
              <div><div className="text-xs text-slate-400">revenue</div><div className="font-medium"><Money value={owner.orgMargin.revenue} /></div></div>
              <div><div className="text-xs text-slate-400">writer cost</div><div className="font-medium"><Money value={owner.orgMargin.writerCost} /></div></div>
              <div><div className="text-xs text-slate-400">margin</div><div className="font-semibold"><Money value={owner.orgMargin.margin} /></div></div>
            </div>
          </Card>

          <Card>
            <h2 className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Profit per writer</h2>
                ...
                {owner.profitPerWriter.map((w) => (
                  <li key={w.writerPartyId} className="flex items-center justify-between gap-3 py-2 text-sm">
                    <div>
                      ...
                      <span className="ml-2 text-xs text-slate-500">{w.jobs} job{w.jobs === 1 ? "" : "s"} · rev <Money value={w.revenue} /> · cost <Money value={w.writerCost} /></span>
                    </div>
          ...
          <Card>
            <h2 className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-500">Clients owing</h2>
                ...
                {owner.duesByClient.map((d) => (
                  <li key={d.clientPartyId} className="flex items-center justify-between gap-3 py-2 text-sm">
                    <div>
                      ...
                      <span className="ml-2 text-xs text-slate-500">invoiced <Money value={d.invoiced} /> · paid <Money value={d.paid} /></span>
                    </div>
      ...
      {dash?.balance && (
        <Card className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-4">
            <div><div className="text-xs text-slate-400">earned (outstanding)</div><div className="font-medium"><Money value={earnings?.outstanding} /></div></div>
            <div><div className="text-xs text-slate-400">earned (total)</div><div className="font-medium"><Money value={earnings?.owed} /></div></div>
            <div><div className="text-xs text-slate-400">dues (outstanding)</div><div className="font-medium"><Money value={charges?.outstanding} /></div></div>
            <div><div className="text-xs text-slate-400">net</div><div className="font-semibold"><Money value={dash.balance.net} /></div></div>
            <div><div className="text-xs text-slate-400">open loops</div><div className="font-medium">{dash.openLoops.count}</div></div>
          </div>
        </Card>
      )}
