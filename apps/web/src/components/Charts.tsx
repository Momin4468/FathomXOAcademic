"use client";
/**
 * Tiny dependency-free SVG charts, shared across planes (business dashboards +
 * Personal Finance). Responsive (100% width via viewBox), clean, mobile-first.
 * No chart library — these three shapes cover the overviews.
 */

/** Distinct, readable category colors. */
export const CHART_PALETTE = [
  "#059669", "#0ea5e9", "#f59e0b", "#f43f5e", "#8b5cf6",
  "#14b8a6", "#84cc16", "#fb923c", "#ec4899", "#6366f1", "#94a3b8",
];

/** Donut for a category breakdown. Renders nothing when empty. */
export function Donut({ slices, size = 132, thickness = 18 }: { slices: { label: string; value: number }[]; size?: number; thickness?: number }) {
  ...
  let start = 0;
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="h-auto w-full max-w-[180px]" role="img" aria-label="Category breakdown">
...
/** Grouped two-series bars per period bucket (income/expense, revenue/cost, …). */
...
/** Net-per-period area/line trend. Needs ≥2 points. */
